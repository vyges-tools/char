...
fn deck_async_q_reset_clears_q_low() {
    ...
    let d = deck_async_q(
        ...
        &["dff.spice".into()],
        &[],
        "X1 CLK D RESET_B VGND VNB VPB VPWR Q DFRTP",
        "CLK",
        "D",
        "Q",
        "RESET_B",
    ...
    assert!(
        d.contains("RASY asys RESET_B 1"),
        "async driven through series R"
    );
    assert!(
        d.contains("VD ds 0 PWL(0 1.8"),
        "reset primes Q=1 (data high)"
    );
    assert!(
        d.contains("TRIG v(RESET_B) VAL=0.9 FALL=1"),
        "active-low assertion falls"
    );
    assert!(
        d.contains("TARG v(Q) VAL=0.9 FALL=1"),
        "reset clears Q (falls)"
    );
}
...
fn deck_async_q_set_drives_q_high() {
    ...
    let d = deck_async_q(
        ...
        &["dff.spice".into()],
        &[],
        "X1 CLK D SET_B VGND VNB VPB VPWR Q DFSTP",
        "CLK",
        "D",
        "Q",
        "SET_B",
    ...
    assert!(
        d.contains("TARG v(Q) VAL=0.9 RISE=1"),
        "set drives Q high (rises)"
    );
}
...
fn render_emits_clear_arc_recovery_removal() {
    ...
    let lib = render_seq(
        ...
        &slews,
        &loads,
        &seq_with(vec![ctl("RESET_B", "!RESET_B", false)]),
    );
...
fn render_emits_preset_for_set() {
    ...
    let lib = render_seq(
        ...
        &slews,
        &loads,
        &seq_with(vec![ctl("SET_B", "!SET_B", true)]),
    );
...
fn render_both_set_and_reset() {
    let (slews, loads) = (vec![0.05], vec![0.005]);
    let cell = seq_with(vec![
        ctl("RESET_B", "!RESET_B", false),
        ctl("SET_B", "!SET_B", true),
    ]);
