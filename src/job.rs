...
pub struct CharJob {
    ...
    pub base_dir: String,
    // Sparse-sweep + surrogate fill (set by `run --sparse`, not the .char file): simulate
    // only `sim_slews × sim_loads` and predict the full grid. None = dense (simulate all).
    pub sparse: Option<SparseCfg>,
    // Per-point sweep parallelism within this cell (set by `run --jobs N`, not the file).
    // 1 = sequential. `library` keeps this 1 (it parallelizes across cells instead).
    pub threads: usize,
    // Self-tuning active sampling (set by `run --auto`): grow the simulated set toward a
    // target accuracy, then surrogate-fill. Mutually exclusive with `sparse`.
    pub auto: Option<AutoCfg>,
}

/// Configuration for a sparse-sweep run: the coarse grid actually simulated, the
/// surrogate polynomial degree, and how many extra points to re-simulate for `--verify`.
#[derive(Debug, Clone)]
pub struct SparseCfg {
    pub sim_slews: Vec<f64>,
    pub sim_loads: Vec<f64>,
    pub degree: usize,
    pub verify: usize,
}

/// Configuration for self-tuning active sampling: start from a `seed` (rows × cols)
/// subset of the dense grid and add the most-informative point until the cross-validated
/// error is within `target_pct` (% of peak) or `max_points` are simulated.
#[derive(Debug, Clone)]
pub struct AutoCfg {
...
impl CharJob {
    pub fn parse(text: &str, base_dir: &str) -> Result<CharJob, JobError> {
        ...
        let job = CharJob {
            ...
            sparse: None, // set by the CLI (`run --sparse`), never the job file
            threads: 1,   // set by the CLI (`run --jobs N`); sequential by default
            auto: None,   // set by the CLI (`run --auto`)
        };
